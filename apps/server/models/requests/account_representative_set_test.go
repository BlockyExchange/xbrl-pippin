...
func TestDecodeAccountRepresentativeSetRequest(t *testing.T) {
	encoded := `{"action":"account_representative_set","wallet":"1234","account":"xbrl_1","representative":"xbrl_2"}`
	...
	assert.Equal(t, "xbrl_1", decoded.Account)
	assert.Equal(t, "xbrl_2", decoded.Representative)
