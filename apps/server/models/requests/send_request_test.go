...
func TestDecodeSendRequest(t *testing.T) {
	encoded := `{"action":"send","wallet":"1234","source":"xbrl_1","destination":"xbrl_2","bpow_key":"abc","amount":"1234"}`
	...
	assert.Equal(t, "xbrl_1", decoded.Source)
	assert.Equal(t, "xbrl_2", decoded.Destination)
...
func TestDecodeSendRequestNumericAmount(t *testing.T) {
	encoded := `{"action":"send","wallet":"1234","source":"xbrl_1","destination":"xbrl_2","bpow_key":"abc","amount":12340000000000000000000000000}`
	var decoded SendRequest
	json.Unmarshal([]byte(encoded), &decoded)
	assert.Equal(t, "send", decoded.Action)
	assert.Equal(t, "1234", decoded.Wallet)
